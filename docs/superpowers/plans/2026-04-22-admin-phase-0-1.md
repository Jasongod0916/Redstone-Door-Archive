...
- [ ] **Step 1: Confirm current public SELECT policy name on `doors`**

Query via the Supabase MCP:

```sql
select polname from pg_policies where schemaname = 'public' and tablename = 'doors' and cmd = 'SELECT';
```

Record the exact policy name. Use it in the `drop policy if exists` line below. The spec guesses `doors_select_public` but the remote baseline may have used a different name.
...
-- ADJUST THE DROP TARGET to the actual policy name from Step 1.
drop policy if exists "<ACTUAL_POLICY_NAME_FROM_STEP_1>" on public.doors;
...
async function tryBootstrap(
  ...
  if (!userEmail || userEmail !== bootstrapEmail) return false
...
function buildUpdatePatch(raw: RawEditablePayload): Record<string, unknown> {
  ...
  const intFields: EditableField[] = ['door_width', 'door_height', 'block_count', 'open_ticks', 'close_ticks', 'total_ticks', 'bounds_width', 'bounds_height', 'bounds_depth']
